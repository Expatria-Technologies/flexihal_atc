...
status_code_t drawbar_open (sys_state_t state, char *args)
{
    ...
    if(   ((atc_status.drawbar_status == 1)     && (atc.flags.drawbar_status_active))
       || ((atc_status.toolpresent_status == 0) && (atc.flags.tool_present_active))) {

        if(atc.flags.air_seal_active)
            hal.port.digital_out(active_ports.air_seal, 0);
...
//
// Administrative only — does not move the machine.  The operator is responsible
// for physically removing the tool from the carousel before issuing this command.
...
// $TCMEASURE — Measure current tool length against G59.3 toolsetter.
// Skips measurement if tool has already been measured (non-zero Z offset).
...
static status_code_t carousel_measure (sys_state_t state, char *args)
{
    ...
    if((sys.homed.mask & (X_AXIS_BIT|Y_AXIS_BIT|Z_AXIS_BIT)) != (X_AXIS_BIT|Y_AXIS_BIT|Z_AXIS_BIT)) {
        report_message("TCMEASURE: machine must be homed", Message_Warning);
        return Status_HomingRequired;
    }

    if(gc_state.tool->tool_id == 0) {
        report_message("TCMEASURE: no tool in spindle", Message_Warning);
        return Status_GcodeValueOutOfRange;
    }
...
static status_code_t carousel_remeasure (sys_state_t state, char *args)
{
    ...
    if((sys.homed.mask & (X_AXIS_BIT|Y_AXIS_BIT|Z_AXIS_BIT)) != (X_AXIS_BIT|Y_AXIS_BIT|Z_AXIS_BIT)) {
        report_message("TCREMEASURE: machine must be homed", Message_Warning);
        return Status_HomingRequired;
    }

    if(gc_state.tool->tool_id == 0) {
        report_message("TCREMEASURE: no tool in spindle", Message_Warning);
        return Status_GcodeValueOutOfRange;
    }
...
#if TOOLTABLE_ENABLE == 2
    { "TCADD",        carousel_add,        { .noargs = Off }, { .str = "Deposit current spindle tool into next free carousel pocket: $TCADD [Tn] [;name]" } },
    { "TCRETURN",     carousel_return,     { .noargs = On  }, { .str = "Return current spindle tool to its carousel pocket" } },
    { "TCRM",         carousel_remove,     { .noargs = Off }, { .str = "Clear tool's carousel pocket assignment (admin only, no motion): $TCRM [Tn]" } },
    { "TCMEASURE",    carousel_measure,    { .noargs = On  }, { .str = "Measure current tool length (skips if already measured)" } },
    { "TCREMEASURE",  carousel_remeasure,  { .noargs = On  }, { .str = "Clear stored offset and re-measure current tool length" } },
#endif
};
...
sys_commands_t *atc_get_commands (void)
{
    return &atc_commands;
}
...
static void onSpindleSetState (spindle_ptrs_t *spindle, spindle_state_t state, float rpm)
{
    ...
    if(state.value != 0) {
        if(   ((atc_status.drawbar_status == 0)     && (atc.flags.drawbar_status_active))
           || ((atc_status.toolpresent_status == 0) && (atc.flags.tool_present_active))
           || ((atc_status.drawbar_control == 1)    && (atc.flags.drawbar_control_active))) {
